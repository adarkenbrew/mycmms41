...
public function page_content() {
    ...
    $tpl->display("tab_object.tpl");
} // EO page_content
public function process_form() {
    $DB=DBC::get();
    try {
        $DB->beginTransaction();
        DBC::execute("UPDATE equip SET EQTYPE=:eqtype,COSTCENTER=:costcenter,DESCRIPTION=:description,SAFETYNOTE=:safetynote WHERE EQNUM=:eqnum",array("eqnum"=>$_REQUEST['EQNUM'],"eqtype"=>$_REQUEST['EQTYPE'],"costcenter"=>$_REQUEST['COSTCENTER'],"description"=>$_REQUEST['DESCRIPTION'],"safetynote"=>$_REQUEST['SAFETYNOTE']));
        $DB->commit();
    } catch (Exception $e) {
        $DB->rollBack();
