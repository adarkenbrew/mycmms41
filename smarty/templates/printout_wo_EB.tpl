...
<tr><td><a href="{$doc_path}{$wo_docs.FILENAME}">{$wo_docs.FILENAME}</a></td><td>{$wo_docs.FILEDESCRIPTION}</td></tr>
</table>
...
{* DLOG *}
<table>
<tr><th>{t}ID{/t}</th><th>{t}Started{/t}</th><th>{t}State{/t}</th><th>{t}DBFLD_DURATION{/t}</th></tr>
{foreach item=dlog from=$dlogs}
<tr><td>{$dlog.ID}</td><td>{$dlog.STARTDATE}</td><td>{$dlog.STATE}</td><td>{$dlog.DURATION}</td></tr>
{/foreach}
</table>
