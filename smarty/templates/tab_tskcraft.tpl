...
<body>
<table width="800">
<tr><th>{t}DBFLD_OPNUM{/t}</th><th>{t}DBFLD_EQNUM{/t}</th><th>{t}DBFLD_OPDESC{/t}</th><th>{t}DBFLD_CRAFT{/t}</th><th>{t}DBFLD_TEAM{/t}</th><th>{t}DBFLD_ESTHRS{/t}</th><th>{t}Action{/t}</th></tr>
...
<input type="hidden" name="ID" value="{$operation.ID}">
    ...
    <td>{include file="_combobox.tpl" options=$crafts NAME="CRAFT" SELECTEDITEM=$operation.CRAFT}</td> 
...
<input type="hidden" name="form_save" value="SET">
    ...
    <td>{include file="_combobox.tpl" options=$crafts NAME="CRAFT" SELECTEDITEM=""}</td> 
