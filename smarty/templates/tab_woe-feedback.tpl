...
<input type="hidden" name="form_save" value="SET">
<input type="hidden" name="ID" value="{$workedhour.ID}">
    <td>{include file="_combobox.tpl" options=$names NAME="EMPCODE" SELECTEDITEM=$workedhour.EMPCODE}</td> 
    <td>{include file="_calendartime.tpl" NAME="WODATE" VALUE=$workedhour.WODATE}</td>
    <td>{$workedhour.ESTHRS}</td>
...
<input type="hidden" name="form_save" value="SET">
<tr bgcolor="{cycle values="WHITE,LIGHTSTEELBLUE"}">
    <td>{include file="_combobox.tpl" options=$names2 NAME="EMPCODE" SELECTEDITEM=$smarty.session.user}</td> 
    <td>{include file="_calendartime.tpl" NAME="WODATE" VALUE=$smarty.now|date_format:"%Y-%m-%d %H:%M"}</td>
